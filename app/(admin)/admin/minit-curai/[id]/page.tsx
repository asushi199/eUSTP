...
export default async function MinitCuraiDetailPage({
  ...
  return (
    <>
      ...
      <h1 className="mt-3 min-w-0 text-2xl font-semibold [overflow-wrap:anywhere]">{report.tajuk}</h1>
      ...
      <dl className="card mt-6 min-w-0 divide-y divide-fog px-5 sm:px-7">
        {fields.slice(0, 8).map(([label, value]) => (
          <div key={label} className="grid min-w-0 grid-cols-1 gap-2 py-4 sm:grid-cols-[210px_minmax(0,1fr)]">
            <dt className="text-sm font-medium text-graphite">{label}</dt>
            <dd className="min-w-0 whitespace-pre-wrap text-sm [overflow-wrap:anywhere]">{value}</dd>
          </div>
        ))}
      </dl>
      <section className="card mt-6 min-w-0 p-5 sm:p-7">
        ...
        <div className="mt-4 space-y-4">
          {report.items.map((item, index) => (
            <article key={`${item.perkara}-${index}`} className="min-w-0 rounded-lg border hairline p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-graphite">Perkara {index + 1}</p>
              <p className="mt-2 min-w-0 whitespace-pre-wrap text-sm [overflow-wrap:anywhere]">{item.perkara}</p>
              <dl className="mt-3 space-y-2 text-sm">
                <div><dt className="text-graphite">Keputusan / penjelasan</dt><dd className="mt-0.5 min-w-0 whitespace-pre-wrap [overflow-wrap:anywhere]">{item.keputusan}</dd></div>
                <div><dt className="text-graphite">Tindakan susulan</dt><dd className="mt-0.5 min-w-0 whitespace-pre-wrap [overflow-wrap:anywhere]">{item.tindakan}</dd></div>
                <div><dt className="text-graphite">Pegawai / unit bertanggungjawab</dt><dd className="mt-0.5 min-w-0 [overflow-wrap:anywhere]">{item.pegawai}</dd></div>
              </dl>
            </article>
          ))}
        </div>
