...
import { ROLE_ORDER, ROLE_INFO, normalizeSchoolCode } from "@/lib/direktori/config";

export const dynamic = "force-dynamic";

export default async function SekolahHistoryPage({
  params,
}: {
  ...
  return (
    <>
      ...
      <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-graphite">
            {school.code} · {school.zone || "tiada zon"}
          </p>
        </div>
      ...
      <div className="mt-3 space-y-3">
        {versions.length === 0 && (
          <div className="card p-6 text-graphite">Belum ada versi untuk sekolah ini.</div>
        )}
        {versions.map((v) => (
              ...
              {ROLE_ORDER.map((role) => {
                ...
                return (
                  <div key={role} className="rounded-lg bg-cloud p-3">
                    ...
                    <p className="mt-1">{contact?.teacherName || "-"}</p>
